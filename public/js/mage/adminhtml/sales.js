...
class AdminOrder {
    ...
    copyDataFromBillingToShipping(fromEl) {
        ...
        const container = document.getElementById(this.shippingAddressContainer);
    ...
    setPaymentMethod(method) {
        if (this.paymentMethod && document.getElementById(`payment_form_${this.paymentMethod}`)) {
            for (const blockId of [`payment_form_${this.paymentMethod}_before`, `payment_form_${this.paymentMethod}_after`]) {
        ...
        if (document.getElementById(`payment_form_${method}`)) {
            for (const blockId of [`payment_form_${method}_before`, `payment_form_${method}_after`]) {
                const blockEl = document.getElementById(blockId);
                if (!blockEl) {
                    continue;
                }
    ...
    submit() {
        if (!this.orderItemChanged) {
            if (!confirm(Translator.translate('You have item changes'))) {
                this.itemsUpdate();
                return;
            }
        }
    ...
    async validateVat(parameters) {
        ...
        try {
            ...
            if (result.valid === true) {
                if (currentCustomerGroupId == result.group) {
    ...
    processCustomerGroupChange(groupIdHtmlId, message, groupId) {
        ...
        const oldGroup = groupSelectEl.querySelector(`:scope > option[value="${groupSelectEl.value}"]`).textContent;
        const newGroup = groupSelectEl.querySelector(`:scope > option[value="${groupId}"]`).textContent;

        const confirmText = message.replace('%s', oldGroup).replace('%s', newGroup);
...
class OrderFormArea {
    ...
    initialize(name, node, parent) {
        ...
        parent[this._callbackName] = wrapFunction(parent[this._callbackName], (proceed) => {
            proceed();
            this.onLoad();
        });
